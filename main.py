...
root = tk.Tk() ; root.title("Gay Meter :O") ; root.geometry("700x500") ; image_path = "assets/asset1" ; image = Image.open(image_path) ; image = ImageTk.PhotoImage(image) ; label_question = tk.Label(root, text="Are you gay ?", font=("Arial", 36)) ; label_question.pack(pady=20)
...
def create_interface():
    ...
    yes_button = CTkButton(root, text="Yes", command=lambda: display_message("I knew it !", "assets/asset2")) ; yes_button.pack(pady=10) ; global no_button ; no_button = CTkButton(root, text="No") ; no_button.pack(pady=10) ; no_button.bind("<Motion>", teleport_no_button)
